(() => {
  ...
  function render() {
    ...
    if($('syncStatus'))$('syncStatus').textContent=state.settings.syncUrl?(syncing?'Syncing…':'Auto-sync ready'):'Local only';
    if($('syncText'))$('syncText').textContent=state.settings.syncUrl?'Auto-pull on open and auto-push after changes.':'Configure Google Apps Script in Settings.';
    theme(); categories();
  }
  ...
  async function sync(action,quiet=false) {
    if(syncing || !state.settings.syncUrl)return;
    syncing=true; render();
    try {
      const data=await api(action,action==='replaceAll'?{transactions:state.transactions,categories:state.categories}:{});
      if(action==='getAll'&&data.data){ state.transactions=Array.isArray(data.data.transactions)?data.data.transactions:[]; if(Array.isArray(data.data.categories)&&data.data.categories.length)state.categories=data.data.categories; save(); render(); }
      if(!quiet)toast(action==='ping'?'Connected':action==='getAll'?'Pulled from Sheet':`Synced ${data.count||0} transactions`);
    } catch(e) { if(!quiet)toast(e.message||'Sync failed'); else console.warn('Automatic sync failed:',e); }
    finally { syncing=false; render(); }
  }
  function autoPull() { if(state.settings.syncUrl) sync('getAll',true); }
  function autoPush() { if(state.settings.syncUrl) sync('replaceAll',true); }
  ...
  if($('clear'))$('clear').addEventListener('click',()=>{if(confirm('Delete all local transactions?')){state.transactions=[];save();render();autoPush();}});
  if($('syncUrl'))$('syncUrl').addEventListener('input',e=>{state.settings.syncUrl=e.target.value.trim();save();render();if(state.settings.syncUrl)autoPull();});
  ...
  if($('addCat'))$('addCat').addEventListener('click',()=>{const name=$('newCat')?.value.trim();if(!name)return toast('Enter a category name.');state.categories.push({name,type:$('newType')?.value||'expense'});$('newCat').value='';save();render();toast('Category added');autoPush();});
